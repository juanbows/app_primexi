...
CREATE INDEX IF NOT EXISTS idx_whoscored_matches_competition_season
    ON whoscored.matches (competition, season);

CREATE INDEX IF NOT EXISTS idx_whoscored_matches_teams
    ON whoscored.matches (home_team_id, away_team_id);

CREATE INDEX IF NOT EXISTS idx_whoscored_matches_external_teams
    ON whoscored.matches (home_whoscored_team_id, away_whoscored_team_id);

CREATE INDEX IF NOT EXISTS idx_whoscored_match_events_order
    ON whoscored.match_events (match_id, period_value, expanded_minute, minute, second, event_seq);

CREATE INDEX IF NOT EXISTS idx_whoscored_match_events_team_time
    ON whoscored.match_events (team_id, match_id, expanded_minute);

CREATE INDEX IF NOT EXISTS idx_whoscored_match_events_external_team_time
    ON whoscored.match_events (whoscored_team_id, match_id, expanded_minute);

CREATE INDEX IF NOT EXISTS idx_whoscored_match_events_type
    ON whoscored.match_events (type_value);

CREATE INDEX IF NOT EXISTS idx_whoscored_match_events_player
    ON whoscored.match_events (player_catalog_id);

CREATE INDEX IF NOT EXISTS idx_whoscored_match_events_external_player
    ON whoscored.match_events (whoscored_player_id);

CREATE INDEX IF NOT EXISTS idx_whoscored_match_events_qualifiers_gin
    ON whoscored.match_events USING GIN (qualifiers_json);

CREATE INDEX IF NOT EXISTS idx_whoscored_event_qualifiers_event
    ON whoscored.event_qualifiers (event_pk);

CREATE INDEX IF NOT EXISTS idx_whoscored_event_qualifiers_type
